...
class v1:
    ...
    def prepare_row(self, row_dict, prev_text, current_text):
        try:
            ...
            if re.search("^(Q[0-9]{3}\.\-|Q[0-9]{3}\-)", current_text):
                # row_dict['course/subject'] = prev_text
                self.current_state = 'question'
                row_dict['question'] = current_text[5:].strip()
                logging.info("Question No. %s=================", current_text[:6])
                return False
            ...
            #     logging.info("Question No. %s=================", current_text[:3])
            #     return False
            #
            ...
            if re.search("^(Q[0-9]{3} |Q [0-9]{3}|Q[0-9]{3}\.)", current_text):
                # row_dict['course/subject'] = prev_text
                self.current_state = 'question'
                row_dict['question'] = current_text[5:].strip()
                logging.info("Question No. %s=================", current_text[:6])
            ...
            if re.search('^Answer: Option', current_text):
                logging.info("answer v1 Answer: Option================= %s", current_text)
                self.current_state = 'answer'
                index = len('Answer: Option')
                row_dict['answer'] = answer[current_text[index + 1:index + 2]]
                return True
            if re.search('^Answer: \(', current_text):
                logging.info("answer v1 Answer: (================= %s", current_text)
                self.current_state = 'answer'
                index = len('Answer: (')
            ...
            if re.search('^Solution :', current_text):
                logging.info("answer v1 Solution :================= %s", current_text)
                self.current_state = 'answer'
                index = len('Solution : ')
                row_dict['answer'] = answer[current_text[index + 1:index + 2]]
                return True
            ...
            #     logging.info("answer v1 [a-d]: ================= %s", current_text)
            #     row_dict['answer'] = answer[current_text[0]]
            #     return True
